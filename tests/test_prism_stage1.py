...
def test_prism_infer_stage1_no_signal_is_a_noop() -> None:
    ...
    def evidence_clean(t: int) -> PrismEvidence:
        return PrismEvidence(
            step_index=t,
            principle_labels=("correct-and-aligned",) * 3,
    ...
    assert result.pred_first_mistake_index is None  # no error trace
...
def test_prism_infer_valid_alternative_suppresses_stage1_signal() -> None:
    ...
    should be SUPPRESSED — this matches our contradiction-strength fusion."""
    num_steps = 4
    ...
    # Step 2 should NOT win because valid-alternative downweights the stage1 signal.
    assert result.pred_first_mistake_index != 2
